...
class ModelCatalogOCFilter extends Model {
  ...
  public function getData(){
		$query = $this->db->query("SELECT DISTINCT ood.option_id AS option_id, ood.name AS option_name, oovd.name AS value, ood.postfix AS postfix, oov.value_id AS value_id FROM " . DB_PREFIX . "ocfilter_option_description ood JOIN " . DB_PREFIX . "ocfilter_option oo ON (ood.option_id = oo.option_id) AND (ood.language_id = '" . (int)$this->config->get('config_language_id') . "')
															...
															JOIN " . DB_PREFIX . "ocfilter_option_value_to_product oovtp ON (oovtp.option_id = oo.option_id) AND (oovd.value_id = oovtp.value_id)");
