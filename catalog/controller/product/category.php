...
class ControllerProductCategory extends Controller {
	public function index() {
		...
		if ($category_info) {
			...
			foreach ($results as $result) {
				if ($result['image']) {
					$image = $this->model_tool_image->resize($result['image'], 230, 178);
				} else {
					$image = $this->model_tool_image->resize('placeholder.png', 230, 178);
				}
				...
				$data['products'][] = array(
					...
					'option'	  => $this->model_catalog_product->getCarsOptions($result['product_id']),
			...
			$pagination->url = $this->url->link('product/category', 'path=' . $this->request->get['path'] . $url . '&page={page}&filter_ocfilter=' . $filter_ocfilter);
